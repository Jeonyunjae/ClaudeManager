...
- iPhone(Safari·홈 화면 설치 앱)에서 의도치 않게 화면이 확대되던 문제 3가지 — (1) 입력 포커스 자동 확대: `MessageComposer` 입력창과 로그인·설정 화면 비밀번호 입력의 글자 크기가 16px 미만(`text-sm`·`fontSize: 14`)이라 iOS가 포커스 시 자동으로 확대하던 것을 16px 이상(`text-base`·`fontSize: 16`)으로 올려 방지(`MessageComposer`의 자동 높이 계산 `lineHeight` 상수도 20→24로 보정, 데스크톱 레이아웃 변화는 로그인·설정 입력 14→16px뿐), (2) 핀치 확대: `viewport`에 `maximumScale: 1`·`userScalable: false` 추가 + iOS Safari가 이를 무시하는 경우를 보강하기 위해 모바일 셸(`/m` layout)에서만 `usePreventZoom()` 훅으로 `gesturestart`/`gesturechange`/`gestureend`를 `preventDefault`(마운트 시 등록, 언마운트 시 해제, 데스크톱 화면에는 미적용), (3) 더블탭 확대: `html`에 `touch-action: manipulation`·`text-size-adjust: 100%` 추가(핀치는 막지 않고 더블탭 확대만 제거) (BUG-028)
